...
public class MoveObject : MonoBehaviour
{
    ...
    public List<Vector3> points; // List of points the object will move through
        ...
        if (_isMoving)
        {
            if (_currentPoint < points.Count)
            {
                ...
                movingObject.transform.position = Vector3.MoveTowards(movingObject.transform.position,
                    points[_currentPoint], speed * Time.deltaTime);
                if (movingObject.transform.position == points[_currentPoint])
                {
