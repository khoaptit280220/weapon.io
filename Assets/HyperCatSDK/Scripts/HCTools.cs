...
public class HCTools : Editor
{
    ...
    {
        var url = "https://raw.githubusercontent.com/duyiosdev/HyperCat/main/GameIDs/Android/" + fileName + ".json";
    ...
    public static void GetMissingNetworks(string[] requiredNetworks, Action<string[]> onComplete)
    {
        ...
        var pluginLoadCoroutine = AppLovinIntegrationManager.Instance.LoadPluginData(p => pluginData = p);

        while (pluginLoadCoroutine.MoveNext())
        {
        }

        if (pluginData == null)
        {
            Debug.LogWarning("Cannot load plugin data");
            return;
        }

        var installedNetworkDisplayNames = new HashSet<string>(pluginData.MediatedNetworks
