...
public class PreBuildCheck : IPreprocessBuildWithReport
{
    ...
    async void CheckAdsId()
    {
...
#if UNITY_ANDROID
        if (AppLovinSettings.Instance.AdMobAndroidAppId != ids.admobId)
            throw new BuildFailedException("Admob ID not match with cloud setting!");
#elif UNITY_IOS
            if (AppLovinSettings.Instance.AdMobIosAppId != ids.admobId)
                throw new BuildFailedException("Admob ID not match with cloud setting!");
#endif
        ...
        if (HCTools.GameSetting.aoa != ids.aoaId)
            throw new BuildFailedException("AOA ID not match with cloud setting!");
    }
}